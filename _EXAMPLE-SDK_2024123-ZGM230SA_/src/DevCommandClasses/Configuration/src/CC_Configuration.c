                                  ...
                                  __attribute__((unused)) const uint8_t cmdLength)
{
  ...
  {
    io_transaction_result = cc_configuration_get(parameter_number, &parameter_buffer);

    if(io_transaction_result == false)
    {
      updated_parameter_counter++;
      continue; 
      // don't attempt to set non-existent parameters, but account for them as part of the "number_of_parameters" function argument
    }

    if(is_default == true)
    {
      cc_configuration_reset_to_default_value(parameter_number);
    ...
    else
    {
      ...
      cc_config_configuration_set_return_value return_value = cc_configuration_set(parameter_number, &new_parameter_value, size);
      if(( CC_CONFIG_RETURN_CODE_IO_FAIL == return_value) || (CC_CONFIG_RETURN_CODE_NOT_SUPPORTED == return_value))
      {
        frame_status = RECEIVED_FRAME_STATUS_FAIL;
        break;
      }
      else if ( CC_CONFIG_RETURN_CODE_OK == return_value)
      {
        at_least_one_success = true;
      }
      cmd_index += size;
                                              ...
                                              const bool flag_handshake)
{
  ...
  uint16_t first_parameter_number = 0;
  ...
  *(p_frame + 6) = DEFAULT_FLAG;
  ...
  for (; count < requested_parameter_count; count++)
  {
    ...
    if (true != status)
    {
      if (0 == parameter_size) // since it is initialized as zero, when this is not true, it was changed
      {
        first_parameter_size = CC_CONFIG_PARAMETER_SIZE_8_BIT;
        // There is no point to search for more than the specified offset, if not found in that range overwrite parameter size
        for (uint8_t Counter = parameter_number; Counter < (parameter_number + requested_parameter_count); Counter++)
        {
          // Since this loop only manages size, it doesn't matter if the data is deposited into the already present parameter_buffer
          // it will get rewriten when the next loop instance is called
          bool tempStatus = cc_configuration_get(Counter, &parameter_buffer);
          if (true == tempStatus) // We found the first valid parameter to extract its size value for all the nonexistent parameters
          {
            first_parameter_size = parameter_buffer.metadata->attributes.size;
            break;
          }
          else if ((Counter + 1) >= (parameter_number + requested_parameter_count)) // && false == tempStatus
          {
            // We didn't find a sinlge valid parameter instance in the parameter offset range, overwrite with the 1st parameter size
            cc_configuration_get(first_parameter_number, &parameter_buffer);
            first_parameter_size = parameter_buffer.metadata->attributes.size;
          }
        }
      }
      parameter_size = first_parameter_size;
    }
    else
    {
  ...
  *(p_frame + 6) |= ((uint8_t)parameter_size & 0x07);
  ...
  for (uint32_t i = 0; i < count; i++)
  {
    ...
    else
    {
      ...
      cc_config_parameter_metadata_t metadata_default = {
        .number = parameter_number,
        .attributes = {
          .name = "Empty Parameter",
          .info = "Empty Parameter",
          .min_value = {0},
          .max_value = {0},
          .size = parameter_size,
        ...
        .migration_handler = NULL // If you don't have a migration handler function
      };

      // Initialize buffer structure
      cc_config_parameter_buffer_t parameter_default = {
        .metadata = &metadata_default,
        .data_buffer = {0},
      };
      parameter_buffer = parameter_default;
    }
...
static bool cc_configuration_write_serial(uint8_t number, uint32_t value)
{
  uint32_t temp[3];

  uint32_t *addr = (uint32_t *)0x0FE00000;

  memcpy(temp, addr, 3*sizeof(uint32_t));

  memcpy((temp + number), &value, sizeof(uint32_t));

  MSC_Init();

  if(mscReturnOk != MSC_ErasePage(addr))
  {
    return false;
  }

  if(mscReturnOk != MSC_WriteWord(addr, temp,  3*sizeof(uint32_t)))
  {
    return false;
  }
...
cc_configuration_set(uint16_t parameter_number,  cc_config_parameter_value_t* new_value, cc_config_parameter_size_t size)
{
  ...
  cc_config_parameter_buffer_t parameter_buffer;
    ...
    bool override_read_only_check = false;
    if((parameter_number == SERIAL_NUMBER_1_PAR || 
        parameter_number == SERIAL_NUMBER_2_PAR || 
        parameter_number == SERIAL_NUMBER_3_PAR) && 
        false == IsSerialLockFlagActive())
    {
      if (false == cc_configuration_write_serial((parameter_number-SERIAL_NUMBER_1_PAR), new_value->as_uint32))
      {
        return CC_CONFIG_RETURN_CODE_IO_FAIL;
      }
      override_read_only_check = true;
    }
    
    for(uint16_t parameter_ix = 0 ; parameter_ix < configuration_pool->numberOfParameters ; parameter_ix++)
    {
      if( (configuration_pool->parameters[parameter_ix].attributes.flags.read_only == false || true == override_read_only_check)
...
cc_configuration_get(uint16_t parameter_number, cc_config_parameter_buffer_t* parameter_buffer)
{
  ...
  if (false == io_transaction_result)
  {
    cc_configuration_get_first_parameter(parameter_buffer);
  }
